...
namespace MultiScreenshotCaptureNamespace
{
	...
	public class MultiScreenshotCapture : EditorWindow
	{
		...
		private const string SESSION_DATA_PATH = "Assets/Packs/MultiScreenshotCapture/MultiScreenshotCapture_Settings.json";
		...
		private static void Init()
		{
			...
			window.titleContent = new GUIContent("Screenshoter");
		...
		private void Awake()
		{
			...
			CreateScreenshoterInput();
		}

		public void CreateScreenshoterInput()
		{
			if (Application.isPlaying)
			{
				if (_screenshoterInput)
				{
					return;
				}

				if (ScreenshoterInput.Instance)
				{
					_screenshoterInput = ScreenshoterInput.Instance;
				}
				else
				{
					GameObject g = new GameObject(name = "ScreenshoterInput");
					_screenshoterInput = g.AddComponent<ScreenshoterInput>();
				}
				
				_screenshoterInput.keyDownEvent.RemoveAllListeners();
				_screenshoterInput.keyDownEvent.AddListener(TakeAScreenshotFromKey);
			}
		}

		private static void TakeAScreenshotFromKey()
		{
			MultiScreenshotCapture window = GetWindow<MultiScreenshotCapture>();
			if (window.GetCurrentActiveField() != "directory name")
			{
				window.TakeScreenshotsButtonPressed();
			}
		}
		...
		private void OnGUI()
		{
			...
			GUI.SetNextControlName("directory name");
			saveDirectory = EditorGUILayout.TextField( "Save directory name:", saveDirectory );

			EditorGUILayout.Space();

			GUI.enabled = queuedScreenshots.Count == 0 && resolutionMultiplier > 0f;
			
			if( GUILayout.Button( "Capture Screenshots (C)" ))
			{
				TakeScreenshotsButtonPressed();
			}
			GUI.enabled = true;
		...
		private string GetCurrentActiveField()
		{
		...
		private void TakeScreenshotsButtonPressed()
		{
			...
			if(!Directory.Exists(Directory.GetCurrentDirectory() + "/" + pickDirectory + "/"))
			{    
				Directory.CreateDirectory(Directory.GetCurrentDirectory() + "/" + pickDirectory + "/");
			}
			...
			{
				Debug.Log( "<b>Saved screenshots:</b> " + Directory.GetCurrentDirectory() + "/" + pickDirectory + "/" );
			}
			else
			{
